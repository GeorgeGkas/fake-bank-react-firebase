...
const Withdrawal = () => {
  ...
  const withdrawal = async () => {
    ...
    const withdrawalAmount = document.getElementById('withdrawal_amount').value

    setRequestEndState(false)
    setLoadingState(true)
    try {
      const response = await axios.patch(
        `${process.env.REACT_APP_API_ENDPOINT}/accounts/withdrawal`,
        {
          id: accountId,
      ...
      if (response.status === 204) {
        setResponseValue(
          `Successfully withdrawal ${withdrawalAmount} coins from account with id "${accountId}".`,
        )
      } else {
